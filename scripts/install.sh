...
if ! $NON_INTERACTIVE; then
  ...
  echo "     Setup auto-detects a supported local planning gateway when one is running."
  echo "     If it reports no live model, rerun it with:"
  echo "       studyloop setup --planning-base-url URL --planning-model MODEL"
  echo "  3. Run 'studyloop web', open Study Plans, then choose Create with Architect."
  echo "     Type or dictate one brain dump. The Architect asks focused follow-up"
  echo "     questions, then lets you approve, change, or reject the exact plan."
  echo "     StudyLoop keeps at most three current plans so your attention stays usable."

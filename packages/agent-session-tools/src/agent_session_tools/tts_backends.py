...
MIRROR WARNING
--------------
``studyloop.tts_backends`` is a byte-for-byte-equivalent copy. The two packages
are independently publishable and must not import each other (see
``config_loader``'s module docstring), so the vocabulary is duplicated on
purpose. ``packages/studyloop/tests/test_voice_backends.py`` asserts the two
stay in step — change one, change both.
"""

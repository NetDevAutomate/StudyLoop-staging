"""studyloop update + upgrade CLI commands."""
...
from studyloop.installers import InstallError, install_agent_definitions, require_repo_root
...
def _detect_package_manager() -> str:
    """Detect the package manager used to install studyloop.

    Probe order: uv -> brew -> pip (fallback).
    """
    # uv tool list includes studyloop if installed via uv tool install
    try:
        r = subprocess.run(
            ["uv", "tool", "list"],
            capture_output=True,
        ...
        if r.returncode == 0 and "studyloop" in r.stdout:
            return "uv"
...
def _upgrade_packages(manager: str, dry_run: bool) -> bool:
    ...
    """
    if manager == "uv":
        cmd = ["uv", "tool", "upgrade", "studyloop"]
    elif manager == "brew":
        cmd = ["brew", "upgrade", "studyloop"]
    else:
        cmd = ["pip", "install", "--upgrade", "studyloop"]
...
def update(ctx: click.Context, as_json: bool) -> None:
    ...
    table = Table(title="studyloop update check", show_lines=False)
    ...
    if has_updates:
        console.print(
            "\n[yellow]Updates available.[/yellow] Run [bold]studyloop upgrade[/bold] to apply."
        )
    else:

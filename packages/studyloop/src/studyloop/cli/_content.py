...
Absorbed from pdf-by-chapters. All commands are under ``studyloop content``.
...
def ingest(
    ...
    from studyloop.content.workflow import build_ingest_plan
    from studyloop.settings import load_settings

    if not dry_run:
        raise click.ClickException(
            "Only 'studyloop content ingest --dry-run' is currently supported. "
...
def import_review(source_dir: Path, course: str, dry_run: bool, as_json: bool) -> None:
    ...
    from studyloop.content.importer import import_review_artefacts
    from studyloop.settings import load_settings
...
def split(source: Path, output_dir: Path, level: int, ranges: str | None) -> None:
    ...
    from studyloop.content.splitter import (
        sanitize_filename,
...
def process(source: Path, output_dir: Path, level: int, notebook_id: str | None) -> None:
    ...
    from studyloop.content.notebooklm_client import upload_chapters
    from studyloop.content.splitter import sanitize_filename, split_pdf_by_chapters
...
def list_cmd(notebook_id: str | None) -> None:
    ...
    from studyloop.content.notebooklm_client import list_notebooks, list_sources
...
def generate(
    ...
    from studyloop.content.notebooklm_client import generate_for_chapters
...
def download(notebook_id: str, output_dir: Path, chapters: str | None) -> None:
    ...
    from studyloop.content.notebooklm_client import download_artifacts
...
def delete_cmd(notebook_id: str) -> None:
    ...
    from studyloop.content.notebooklm_client import delete_notebook
...
def syllabus(
    ...
    from studyloop.content.notebooklm_client import create_syllabus, list_sources
    from studyloop.content.syllabus import (
        SyllabusChunk,
        SyllabusState,
    ...
    async def _generate_syllabus() -> dict[int, SyllabusChunk]:
        from studyloop.content.notebooklm_client import _import_notebooklm
    ...
    console.print(f"Next: Run [bold]studyloop content autopilot -o {output_dir}[/bold]")
...
def autopilot(output_dir: Path, book_name: str | None, timeout: int) -> None:
    ...
    from studyloop.content.notebooklm_client import (
    ...
    if not state:
        raise click.ClickException(
            f"No syllabus state found at {state_path}. Run 'studyloop content syllabus' first."
        )
...
def from_obsidian(
    ...
    from studyloop.content.markdown_converter import convert_directory
    from studyloop.content.notebooklm_client import (
        download_artifacts,
        generate_for_chapters,
        upload_chapters,

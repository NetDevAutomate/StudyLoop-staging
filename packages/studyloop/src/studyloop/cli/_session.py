...
def session_start(topic: str, energy: int) -> None:
    ...
    if is_session_active():
        console.print(
            ...
            "  Check:  [bold]studyloop session status[/bold]\n"
            "  End:    [bold]studyloop session end[/bold]"
        )
        raise SystemExit(1)
    ...
    if not study_id:
        console.print("[red]Failed to start session. Run 'studyloop doctor' to diagnose.[/red]")
        raise SystemExit(1)
...
def session_end(notes: str) -> None:
    ...
    from studyloop.history import end_study_session
...
def session_status() -> None:
    ...
    if not state.get("study_session_id"):
        console.print(
            "[dim]No active session. Start one with 'studyloop session start -t <topic>'[/dim]"
        )
        return
...
def session_effectiveness(persona: str | None) -> None:
    ...
    Examples:
        ...
        studyloop session effectiveness --persona 25a67207bce5145d
    """
    from studyloop.history.sessions import get_persona_effectiveness
...
def topic_cmd(name: str, status: str, note: str) -> None:
    ...
    state = read_session_state()
    if not state.get("study_session_id"):
        console.print("[yellow]No active session. Start one with 'studyloop study'.[/yellow]")
        return
...
def park(question: str, topic: str | None, context: str | None) -> None:
    ...
    from studyloop.parking import park_topic
    from studyloop.session_state import append_parking, read_session_state
    ...
    if row_id:
        ...
        console.print("[red]Failed to park topic. Run 'studyloop doctor' to diagnose.[/red]")

...
from studyloop.settings import CONFIG_DIR, get_config_path
...
def setup() -> None:
    """First-run setup wizard — configure studyloop in under 2 minutes."""
    ...
    console.print(
        "studyloop is your AuDHD study pipeline: it syncs Obsidian notes to "
    ...
    if has_ai:
        ...
        console.print(
            f"  [dim]Noted. Run 'studyloop config init' to install agent definitions "
            f"for {assistant}.[/dim]\n"
        )
    else:
        console.print("  [dim]No problem — studyloop works fine as a standalone CLI.[/dim]\n")
    ...
    # ------------------------------------------------------------------
    console.print()
    launch = click.confirm("Launch the studyloop web UI now?", default=False)
    if launch:
        ...
        try:
            ...
            console.print("[yellow]Could not launch web UI. Run 'studyloop web' manually.[/yellow]")
    else:
        console.print("\n[bold]You're all set.[/bold] Next steps:")
        console.print("  studyloop config show    — review your configuration")
        console.print("  studyloop review         — see what topics are due for review")
        console.print("  studyloop web            — open the web UI")
        console.print("  studyloop --help         — explore all commands")

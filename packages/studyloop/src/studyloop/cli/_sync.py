...
from studyloop.cli._shared import console, get_topic
from studyloop.maintenance import dedup_notebook, find_duplicates
from studyloop.state import SyncState
from studyloop.sync import find_changed_sources, find_sources, generate_audio, sync_topic
...
def audio(topic_name: str, instructions: str) -> None:
    ...
    if not ts.notebook_id:
        console.print("[red]Notebook not created yet. Run 'studyloop sync' first.[/red]")
        raise SystemExit(1)

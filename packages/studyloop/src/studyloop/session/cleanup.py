...
def _persist_session_data(
    ...
    *,
    ...
    if auto_persist:
        try:
            from studyloop.services.backlog import auto_persist_struggled
    ...
    try:
        ...
        if topic_slug and topic_entries:
            from studyloop.services.flashcard_writer import write_session_flashcards
...
def _kill_background_processes(state: dict) -> None:
    ...
    if web_port:
        with contextlib.suppress(Exception):
            _kill_port_occupant(int(web_port), expected_cmd="studyloop")

...
def ensure_notebook(topic: Topic, state: SyncState) -> str:
    ...
    try:
        ...
        print(f"[studyloop] Failed to parse notebook list: {result.stdout[:200]}", file=sys.stderr)
        notebooks = []
    for nb in notebooks:
    ...
    try:
        ...
        print(
            f"[studyloop] Failed to parse create response: {result.stdout[:200]}",
            file=sys.stderr,
        )
        data = {}
...
def generate_audio(topic: Topic, state: SyncState, instructions: str = "") -> str | None:
    ...
    try:
        ...
        print(
            f"[studyloop] Failed to parse audio response: {result.stdout[:200]}",
            file=sys.stderr,
        )
        return None

...
from studyloop.logic.briefing_logic import (
    BriefingData,
    ContentContext,
    ReviewContext,
...
class TestFormatContentSection:
    ...
    def test_shows_content_gap_hint_when_no_chapters(self):
        ...
        result = format_study_briefing(data)
        assert "studyloop content split" in result
...
class TestGatherReviewContext:
    def test_returns_none_on_db_error(self):
        ...
        with patch("studyloop.services.review.get_stats", side_effect=RuntimeError("DB down")):
            # Import the gatherer from the CLI module
            from studyloop.session.start import _gather_review_context
    ...
    def test_returns_review_context_on_success(self):
        ...
        with (
            patch(
                "studyloop.services.review.get_stats",
                return_value={
                    "mastered": 5,
                    "total_reviews": 50,
                    "flashcard_count": 20,
            ...
            patch("studyloop.services.review.get_due", return_value=[mock_card, mock_card]),
        ):
            from studyloop.session.start import _gather_review_context

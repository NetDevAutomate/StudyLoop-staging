...
from studyloop.content.notebooklm_client import (  # noqa: E402
    create_syllabus,
    delete_notebook,
    download_artifacts,
...
def _no_real_sleep():
    with patch("studyloop.content.notebooklm_client.asyncio.sleep", new_callable=AsyncMock):
        yield
...
def patch_notebooklm(mock_notebooklm_client):
    ...
    with patch(
        "studyloop.content.notebooklm_client.NotebookLMClient.from_storage",
...
class TestGenerateForChapters:
    ...
    @patch("studyloop.content.notebooklm_client.asyncio.sleep", new_callable=AsyncMock)

"""Tests for studyloop update and upgrade CLI commands."""
...
class TestUpdateCommand:
    ...
    def test_update_shows_versions(self, runner: CliRunner):
        ...
        mock_results = [
            CheckResult(
                "updates", "update_studyloop", "warn", "2.0.0 -> 2.1.0", "studyloop upgrade", True
            ),
            CheckResult("core", "studyloop_installed", "pass", "studyloop 2.0.0", "", False),
        ]

        with patch("studyloop.cli._upgrade._get_registry") as mock_reg:
    ...
    def test_update_json(self, runner: CliRunner):
        ...
        mock_results = [
            CheckResult(
                "updates", "update_studyloop", "pass", "studyloop 2.0.0 (latest)", "", False
            ),
        ]
    ...
    def test_update_always_exit_0(self, runner: CliRunner):
        ...
        mock_results = [
            CheckResult(
                "updates", "update_studyloop", "warn", "update available", "studyloop upgrade", True
            ),
        ]
...
class TestPackageManagerDetection:
    def test_detect_uv(self):
        from studyloop.cli._upgrade import _detect_package_manager

        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="studyloop 2.0.0\n")
    ...
    def test_detect_brew(self):
        ...
        def side_effect(cmd, **kw):
            ...
            return MagicMock(returncode=0, stdout="studyloop\n")
...
class TestDbBackup:
    def test_backup_created(self, tmp_path: Path):
        from studyloop.cli._upgrade import _backup_database
    ...
    def test_backup_pruning(self, tmp_path: Path):
        from studyloop.cli._upgrade import _prune_old_backups
...
class TestUpgradeCommand:
    ...
    def test_dry_run(self, runner: CliRunner):
        from studyloop.cli._upgrade import upgrade
    ...
    def test_agents_component_runs_refresh(self, runner: CliRunner):
        ...
        with (
            patch("studyloop.cli._upgrade._get_registry") as mock_reg,
            patch("studyloop.cli._upgrade._upgrade_agents", return_value=True) as mock_upgrade,
        ):
            mock_reg.return_value.run_all.return_value = mock_results

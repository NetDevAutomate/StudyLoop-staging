...
These tests replicate exactly what a user does: spawn studyloop study,
...
class TestNestedTmux:
    ...
    def test_nested_study_creates_session(self, terminal, tmp_path):
        """From inside a host tmux session, studyloop study creates a study
...
class TestEndFromOutside:
    """Task 6: Verify studyloop study --end works from a separate terminal.
    ...
    def test_end_kills_tmux_session(self, terminal, tmp_path):
        """studyloop study --end from outside should kill the tmux session."""
        agent_cmd = long_running_agent(tmp_path)

...
class TestPypiVersionCheck:
    def test_up_to_date(self):
        ...
        with (
            patch("studyloop.doctor.updates._get_installed_version", return_value="2.0.0"),
            patch("studyloop.doctor.updates._read_cache", return_value=None),
            patch("urllib.request.urlopen", return_value=mock_resp),
            patch("studyloop.doctor.updates._write_cache"),
        ):
            results = check_pypi_versions()
        assert any(r.status == "pass" and "studyloop" in r.name for r in results)
    ...
    def test_cache_hit_skips_network(self):
        ...
        cached = {"studyloop": "2.0.0", "agent-session-tools": "1.0.0"}

        with (
            patch("studyloop.doctor.updates._get_installed_version", return_value="2.0.0"),
            patch("studyloop.doctor.updates._read_cache", return_value=cached),
        ):
            results = check_pypi_versions()
...
class TestCache:
    def test_write_and_read(self, tmp_path: Path):
        from studyloop.doctor.updates import _read_cache, _write_cache

        cache_file = tmp_path / "pypi-check.json"
        data = {"studyloop": "2.0.0"}
        with patch("studyloop.doctor.updates._get_cache_path", return_value=cache_file):
            _write_cache(data)
            result = _read_cache()
        assert result == data

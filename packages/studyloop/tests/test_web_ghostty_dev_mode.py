...
class TestWtermRegressionGuard:
    """T4.4: wterm still works when explicitly selected via --dev-renderer wterm."""

    def test_wterm_loads_with_explicit_flag(self, browser) -> None:
        """--dev-renderer wterm still activates the wterm adapter."""
        port = WEB_GHOSTTY_PORT + 2  # 18582
        ...
        try:
            ...
            try:
                ...
                # wterm meta tag should be present
                content = page.eval_on_selector(
                    'meta[name="studyloop-dev-mode"]',
                    "(el) => el.getAttribute('content')",
                )
                assert content == "wterm", f"Expected wterm meta, got {content!r}"

                # WtermLib should be loaded
                page.wait_for_function(
                    "() => typeof window.WtermLib !== 'undefined'",
                    timeout=5000,
                )

                # Terminal should be WTermAdapter
                terminal_name = page.evaluate("() => window.Terminal.name")
                assert terminal_name == "WTermAdapter", (
                    f"Expected WTermAdapter, got {terminal_name!r}"
                )
            finally:
                page.close()
                context.close()

...
class TestRegistry:
    ...
    def test_get_adapter_unknown_raises(self):
        from studyloop.adapters.registry import get_adapter, reset_registry
    ...
    def test_detect_agents_env_override(self):
        ...
        from studyloop.adapters._protocol import AgentAdapter
        from studyloop.adapters.registry import detect_agents, reset_registry
    ...
    def test_get_default_agent_returns_first_available(self):
        ...
        with (
            patch("studyloop.adapters.registry.shutil.which", return_value="/usr/bin/alpha"),
    ...
    def test_get_default_agent_returns_none_when_empty(self):
        ...
        with (
            patch("studyloop.adapters.registry.shutil.which", return_value=None),
            patch("studyloop.settings.load_settings", return_value=settings_mock),
        ):
            result = registry.get_default_agent()
...
class TestDetectAgentsPriority:
    def test_respects_config_priority_order(self):
        ...
        with (
            patch("studyloop.adapters.registry.shutil.which", side_effect=which_all),
            patch("studyloop.settings.load_settings", return_value=settings_mock),
    ...
    def test_unavailable_agents_excluded(self):
        ...
        with (
            patch("studyloop.adapters.registry.shutil.which", side_effect=selective_which),
            patch("studyloop.settings.load_settings", return_value=settings_mock),
            patch.dict("os.environ", {}, clear=True),
        ):
            result = registry.detect_agents()
    ...
    def test_settings_failure_falls_back_gracefully(self):
        ...
        with (
            patch("studyloop.adapters.registry.shutil.which", return_value="/usr/bin/alpha-bin"),
            patch("studyloop.settings.load_settings", side_effect=RuntimeError("boom")),
            patch.dict("os.environ", {}, clear=True),
        ):
            result = registry.detect_agents()
...
class TestDiscoveryErrorHandling:
    def test_bad_module_logged_and_skipped(self, caplog):
        ...
        with (
            ...
            caplog.at_level(logging.WARNING, logger="studyloop.adapters.registry"),
        ):
    ...
    def test_custom_agents_merged(self):
        ...
        with (
            patch(
                "studyloop.adapters.registry._load_custom_agents",
                return_value={"custom-agent": custom},
            ),
            patch("studyloop.adapters.registry._discover_builtins", return_value={}),
        ):
    ...
    def test_custom_cannot_override_admitted_harness(self):
        ...
        with (
            patch(
                "studyloop.adapters.registry._discover_builtins",
    ...
    def test_load_custom_agents_returns_empty_when_loader_raises(self):
        ...
        with patch.dict("sys.modules", {"studyloop.adapters._custom": fake_module}):
            result = registry._load_custom_agents()
        assert result == {}

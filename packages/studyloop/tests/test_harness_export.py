...
class TestMandate:
    def test_writes_mandate_with_correct_flags(self, repo_root: Path, home: Path):
        result = I.install_session_db_mandate(repo_root, tools=["claude", "kiro"])
        ...
        assert I._MANDATE_SENTINEL in claude_text

    def test_idempotent(self, repo_root: Path, home: Path):
        I.install_session_db_mandate(repo_root, tools=["claude"])
        again = I.install_session_db_mandate(repo_root, tools=["claude"])
    ...
    def test_skips_unknown_harness(self, repo_root: Path, home: Path):
        ...
        result = I.install_session_db_mandate(repo_root, tools=["codex"])
        assert result == {}
...
class TestClaudeStopHook:
    ...
    def test_adds_hook_preserving_existing(self, home: Path):
        ...
        assert I.install_claude_stop_hook() == 1
...
class TestHarnessDoctorCheck:
    def test_warns_when_unwired_then_passes_after_fix(self, repo_root: Path, home: Path):
        ...
        with patch.object(I, "detect_available_agent_tools", return_value=["claude", "kiro"]):
        ...
        # Apply the fix.
        I.install_session_db_mandate(repo_root, tools=["claude", "kiro"])
        I.install_claude_stop_hook()
    ...
    def test_results_are_auto_fixable_warnings(self, home: Path):
        ...
        with patch.object(I, "detect_available_agent_tools", return_value=["kiro"]):

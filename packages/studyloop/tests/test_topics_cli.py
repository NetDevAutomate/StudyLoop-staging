"""Tests for studyloop topics CLI commands (imperative shell)."""
...
class TestTopicsList:
    def test_empty_backlog(self):
        from studyloop.cli._topics import topics_group

        with patch("studyloop.parking.get_parked_topics", return_value=[]):
    ...
    def test_shows_topics_table(self):
        ...
        with patch("studyloop.parking.get_parked_topics", return_value=mock_data):
            result = _runner().invoke(topics_group, ["list"])
    ...
    def test_filter_by_tech(self):
        ...
        with patch("studyloop.parking.get_parked_topics", return_value=[]) as mock_get:
...
class TestTopicsAdd:
    def test_add_success(self):
        ...
        with patch("studyloop.parking.park_topic", return_value=42):
    ...
    def test_add_with_note(self):
        ...
        with patch("studyloop.parking.park_topic", return_value=43) as mock_park:
            _runner().invoke(
                topics_group,
    ...
    def test_add_failure(self):
        ...
        with patch("studyloop.parking.park_topic", return_value=None):
...
class TestTopicsResolve:
    def test_resolve_success(self):
        ...
        with patch("studyloop.parking.resolve_parked_topic", return_value=True):
    ...
    def test_resolve_not_found(self):
        ...
        with patch("studyloop.parking.resolve_parked_topic", return_value=False):
...
class TestTopicsSuggest:
    ...
    def test_shows_ranked_suggestions(self):
        ...
        with (
            patch("studyloop.parking.get_parked_topics", return_value=mock_data),
            patch(
                "studyloop.parking.get_topic_frequencies",

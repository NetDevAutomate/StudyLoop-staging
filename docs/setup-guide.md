...
# Prompts securely before any agent starts. Leave blank to generate a password:
...
The reusable password is never accepted in process arguments, agent-readable
settings, or session state. Existing `lan_password` config is migrated
atomically to an opaque one-way `lan_password_verifier` before any agent starts.
Without a migrated verifier, the command requires an interactive terminal;
...
lan_username: study
lan_password_verifier: ""  # opaque one-way value; do not put a plaintext password here

...
| Card / quiz generation is CLI-only (`studyloop content generate-cards`) | Generate panel exposes the existing pipeline over HTTP+WS with a sidebar UI; provider registry covers **seven** providers — OpenAI/OpenRouter/Gemini/MiniMax/Anthropic via two adapter classes, plus **Bedrock and Ollama as first-class registry entries** (shipped 2026-06-01) | Mid-session "mark this concept for review" → instant deck targeted at the marked concepts (v2 of the generation panel; needs new MCP tool calls from the agent) |

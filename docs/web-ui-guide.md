...
Speech is synthesised on a Kokoro server you run, not in the browser: the page posts the text to StudyLoop's own password-protected `/api/tts/speak`. StudyLoop tries the configured primary, then its fallback list (VoiceMode on port 8880 by default). The device only plays the audio it gets back, which is why a tablet on the LAN gets the same voice as the desktop. With no server reachable the app falls back to your operating system's voices, and failing that to silence. Full details, including how to disable or change the fallback and the LAN-exposure trade-off: [Voice Output § Web App Voice](voice-output.md#web-app-voice-server-side-kokoro).
